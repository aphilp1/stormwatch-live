#!/usr/bin/env python3
"""
merge_woolsey_dem.py
====================
Merge DEM features from hrrr_error_dataset_dem.csv into hrrr_error_dataset.csv
for all woolsey_2018 rows with NEEDS_DEM values.

Note: CUUC1 is NOT in hrrr_error_dataset_dem.csv (no DEM entry) — it will
remain NEEDS_DEM after this merge. All other 16 active woolsey rows are merged.
"""
...
# Backup
backup = MAIN.replace('.csv', '_pre_woolsey_dem.csv')
...
for r in all_rows:
    ...
    key = (r['stid'], r['event_id'])
    if key not in dem_vals:
        print(f'  SKIP (no DEM entry): {r["stid"]}')
        n_skipped += 1
        continue
...
print(f'Merged DEM for {n_updated} rows. Skipped (no DEM entry): {n_skipped}.')
...
print(f'NEEDS_DEM remaining in active woolsey rows: {len(remaining)}')
if remaining:
    print(f'  Still NEEDS_DEM: {[r["stid"] for r in remaining]}')
