...
- **Source of truth:** the live station registry (Synoptic/MesoWest metadata API or
  ...
  stations and *what they observed*, not where the sensor sits. (Correction 2026-05-30:
  Brewer & Clements Table 1 IS the station description table with Lat/Lon, Elevation,
  Type, RecordSpan — confirmed via PDF extraction. Jarbo 39.74/-121.49/773m cross-
  validates against NIFC exactly. However, paper coords are rounded to 2 decimal places;
  the live registry remains the authoritative source for full-precision coordinates.)
